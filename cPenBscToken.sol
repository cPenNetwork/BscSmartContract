...
// File contracts/cPenBscToken_Dev.sol
...
contract CPenBscToken is ERC20, Ownable {
    ...
    /**
     * @dev Exclusions from limits
     */
    mapping(address => bool) private _limitExclusions;

    /**
     * @dev Router restrictions
     */
    ...
    event BatchAirdrop(uint256 totalAmount, uint256 recipientCount);
    ...
    event SetUniswapRouter(address indexed router, bool indexed flag);
    event EnableTrading();
    ...
    /**
     * @dev Bot management functions
     */
    function markBot(address wallet, bool isBot) external onlyOwner {
        bots[wallet] = isBot;
        emit BotStatusUpdated(wallet, isBot);
    }

    function markBotBatch(address[] calldata wallets, bool isBot) external onlyOwner {
        uint256 length = wallets.length;
        for (uint256 i = 0; i < length;) {
            bots[wallets[i]] = isBot;
            unchecked { ++i; }
        }
        emit MultipleBotStatusUpdated(length, isBot);
    }

    /**
     * @dev Router management functions
     */
    ...
    function isTradingEnabled() public view returns (bool) {
        return _tradingEnabledBlock != 0;
    }

    /**
     * @dev Limit exclusion management
     */
    ...
    function batchAirdrop(
        ...
        emit BatchAirdrop(totalAmount, length);
    }

    function _checkBotStatus(address from, address to) internal view {
    ...
    function _checkTransferDelay(address from, address to) internal {
        if (from != address(0)) {
            if (!_limitExclusions[from] && !_limitExclusions[to]) {
                // Require 2 blocks to pass between transfers
                if (_holderLastTransferTimestamp[tx.origin] >= block.number - 2 || 
    ...
    /**
     * @dev Override _update function from ERC20
     */
    function _update(
        address from,
        address to,
        ...
        // Check trading status for router transactions
        if (!isTradingEnabled() && _uniswapRouters[msg.sender]) {
            if (!_limitExclusions[from] && !_limitExclusions[to]) {
                revert TradingNotActive();
            }
        }
              
        _checkBotStatus(from, to);
